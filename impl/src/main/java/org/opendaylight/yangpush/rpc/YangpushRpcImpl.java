...
/**
 *  This class implements RPC defined in ietf-datastore-push@2015-10-15.yang
 *  yang model. Also registers the Notification listener for the notification
 *  defined in the model.
 *
 *  This is the BI implementation of handling all RPC. This uses BI because
 *  anyxml support is not available in BA approach. The subtree filter
 *  argument handled in anyxml type.
 *
 *  RPC: Create-subscription implementation details:
 *  ---------------------------
 *  The basic principle of registering a subscription is:
 *
 *  Uses create-subscription RPC present in ietf-datastore-push module.
 *  The input of the create-subscription RPC augmented by yangpush module
 *  to add mountpoint device name to input parameter.
 *
...
public class YangpushRpcImpl implements DOMRpcImplementation {
    ...
    //QName and NodeIdentifier for input args for create-subscription RPC present in notification.yang
    public static final NodeIdentifier I_RPC_CS_INPUT = NodeIdentifier.create(QName.create(NOTIFICATION_NS, NOTIFICATION_NS_DATE, "input"));
    ...
    //Node identifier for create-subscription RPC present in ietf-datatsore-push.yang
    ...
    public YangpushRpcImpl(DOMRpcProviderService service,
    ...
    private void registerRPCs() {
        ...
        service.registerRpcImplementation(this,
                CREATE_SUBSCRIPTION_RPC,
                MODIFY_SUBSCRIPTION_RPC,
    ...
    /**
     * This method handles createSubscription RPC call.
     * Parse the input and based on that, creates subscription.
     * and returns subscription-id for the subscription.
     * If the subscription Id is -1 then there is error in subscriotion
     * creation. Else a valid subscription id generated by this method.
     *
     * Subscription-id : "-1" -> represents error in subscription
     * Subscription-id : "yp-<integer>" -> represents the subscription id
     *                   created. This subscription-id should be used for
     *                   all further communication related to the subscription.
     * @param input
     * @return
     */
    private CheckedFuture<DOMRpcResult, DOMRpcException> createSubscriptionRpcHandler(NormalizedNode<?, ?> input) {
        String error = "";//new String();
        String sid = "sub-"+Integer.toString(sub_id);
        ...
        YangpushDOMNotificationListener listener = new YangpushDOMNotificationListener(
                this.globalDomDataBroker,
                inputData.getSubscription_id());
    ...
    /**
     * This method parse the input arguments for create-subscription RPC.
     * The expected format of RPC input from user for create-subscription is:
     *
     *
     ...
     * </input>
     *
     * @param input : create-subscription RPC input.
     *                input expects augment node defined in yangpush.yang
     * @param error : returns error sting if there is any issue in input.
...
/*                streamNode = conNode.getChild(stream).get();
     ...
     * create-subscription rpc present in notification.yang model.
     * This input arguments augmented by yangpush to add subscription-id and
     * period leaf nodes
     *
